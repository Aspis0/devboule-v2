...
The design doctrine under `src/features/design/craft/` is adapted prose. It is condensed and rewritten from the MIT-licensed [refero_skill](https://github.com/referodesign/refero_skill) project (Copyright (c) 2026 Refero), taken from `master` at commit a9b54a3e62a6391f5f5ab7a20e4ddb32fb79a27d on 2026-09-06. No upstream file is copied. Every section here is capped at 2,500 characters by the loader's strict first-party check, against upstream files of 7,218 to 14,435 bytes; the cap is enforced rather than asserted, so the ratio cannot drift as the prose is edited. The references to the upstream's own linter and research tooling were dropped, and the rules were rewritten to name no palette, because the text guides code written into a user's project rather than into this one. Some of the material has no refero_skill counterpart and comes instead from [OpenDesign](https://github.com/nexu-io/open-design) at commit 3d0d15fc55031e8e6cead709491e7b82565c4dee, whose `craft/` directory is itself an adaptation of the same refero_skill material; it is Apache-2.0 licensed, Copyright 2026 Open Design contributors, and carries no NOTICE file and no per-file headers. From it come the whole of `state-coverage.md`, condensed from a 7,218-byte source that itself credits WCAG 2.2, NN/g, Material Design 3, Apple HIG and Baymard Institute research; two rules in `anti-ai-slop.md`, against invented metrics and against filler copy; and two in `color.md` — the mechanics of an undefined custom property, from `packages/contracts/src/design-systems/token-schema.ts`, and the cap of two visible accent uses per screen, which refero states only as "sparingly". `LICENSE.refero_skill` ships beside the material with the upstream MIT terms, and `VERSION` records both origins with their commits and the date taken. The files are read as text and composed into a prompt; nothing in them is executed. This is prose rather than library source code, so the statement above still holds.
