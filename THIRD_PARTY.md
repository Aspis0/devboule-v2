...
The design doctrine under `src/features/design/craft/` is adapted prose. It is condensed and rewritten from the MIT-licensed [refero_skill](https://github.com/referodesign/refero_skill) project (Copyright (c) 2026 Refero), taken from `master` at commit a9b54a3e62a6391f5f5ab7a20e4ddb32fb79a27d on 2026-09-06. No upstream file is copied. The upstream files are 13,318 and 14,435 bytes and each section here is under a fifth of that, which the loader enforces rather than merely reports: its strict first-party check caps a section at 2,500 characters. The references to the upstream's own linter and research tooling were dropped, and the rules were rewritten to name no palette, because the text guides code written into a user's project rather than into this one. Two rules in `anti-ai-slop.md`, against invented metrics and against filler copy, do not appear in refero_skill and were taken instead from [OpenDesign](https://github.com/nexu-io/open-design) at commit 3d0d15fc55031e8e6cead709491e7b82565c4dee, whose `craft/` directory is itself an adaptation of the same refero_skill material; it is Apache-2.0 licensed, Copyright 2026 Open Design contributors, and carries no NOTICE file and no per-file headers. `LICENSE.refero_skill` ships beside the material with the upstream MIT terms, and `VERSION` records both origins with their commits and the date taken. The files are read as text and composed into a prompt; nothing in them is executed. This is prose rather than library source code, so the statement above still holds.
