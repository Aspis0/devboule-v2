...
/// The `acp_sessions` flake's producer-side order: the roster push carrying
/// a raise must be prepared before the published event can wake its
/// observer. The hooks below stand in for the registry's real ones: the
/// fake `prepare` records the observer generation where the roster snapshot
/// is captured, and the fake push records it again where the push is
/// queued. Both must read the pre-publish baseline — a wake that slipped in
/// first would already have bumped it.
struct AttentionOrderProbe {
    runtime: SessionRuntime,
    baseline: u64,
    log: Arc<Mutex<Vec<(&'static str, u64)>>>,
}

fn attention_order_probe() -> AttentionOrderProbe {
    let runtime = SessionRuntime::new();
    let conn = ConnHandle::new(1);
    runtime
        .try_attach_with_replay(None, &conn, false)
        .expect("the probe connection attaches");
    let baseline = conn.outbound.wake_generation();
    let log: Arc<Mutex<Vec<(&'static str, u64)>>> = Arc::new(Mutex::new(Vec::new()));
    let prepare_log = Arc::clone(&log);
    let prepare_outbound = Arc::clone(&conn.outbound);
    let push_log = Arc::clone(&log);
    let push_outbound = Arc::clone(&conn.outbound);
    runtime.set_attention_hooks(
        Arc::new(|| false),
        Arc::new(move || -> Box<dyn FnOnce() + Send> {
            prepare_log
                .lock()
                .expect("order log")
                .push(("prepare", prepare_outbound.wake_generation()));
            // The hook is `Fn`: clone before moving into the one-shot push.
            let push_log = Arc::clone(&push_log);
            let push_outbound = Arc::clone(&push_outbound);
            Box::new(move || {
                push_log
                    .lock()
                    .expect("order log")
                    .push(("push", push_outbound.wake_generation()));
            })
        }),
    );
    AttentionOrderProbe {
...
fn assert_attention_queued_before_event_wake(probe: &AttentionOrderProbe, context: &str) {
    let log = probe.log.lock().expect("order log");
    assert_eq!(
        log.as_slice(),
        &[("prepare", probe.baseline), ("push", probe.baseline)],
        "the roster snapshot carrying the raise is prepared and queued before the event wakes its observer ({context})"
    );
    let attention = probe.runtime.attention.lock().expect("attention");
    assert!(
        matches!(*attention, Some(raised) if matches!(raised.reason, AttentionReason::Error)),
        "the published error raised error attention ({context}): {attention:?}"
    );
}

/// The live ACP path: the malformed-output `AgentError` must raise before
/// the event that names it can wake a client. With the old order the wake
/// ran first and both entries read one past the baseline.
#[test]
fn error_attention_is_queued_before_the_event_wakes_its_observer() {
    let probe = attention_order_probe();
    probe.runtime.publish_agent_event(
        SessionEvent::AgentError {
            message: "malformed ACP output was skipped".to_string(),
        },
        None,
    );
    assert_attention_queued_before_event_wake(&probe, "publish_agent_event");
}

/// The journaled twin of the same publish: same order, same assertion.
#[test]
fn journaled_error_attention_is_queued_before_the_event_wakes_its_observer() {
    let probe = attention_order_probe();
    probe
        .runtime
        .publish_agent_error("malformed ACP output was skipped".to_string());
    assert_attention_queued_before_event_wake(&probe, "publish_agent_error");
}
