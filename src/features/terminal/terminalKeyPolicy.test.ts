...
describe('terminalKeyPolicy', () => {
  ...
  it('passes copy and AltGr variants through', () => {
    const onCtrlC = vi.fn();
    expect(terminalKeyPolicy({ type: 'keydown', ctrlKey: true, shiftKey: true, key: 'C' }, onCtrlC)).toBe(true);
    expect(terminalKeyPolicy({ type: 'keydown', ctrlKey: true, altKey: true, key: 'c' }, onCtrlC)).toBe(true);
