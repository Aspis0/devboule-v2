...
export const MOCK_PROVIDER_MANIFESTS: readonly MockProviderManifest[] = [
  {
    ...
    modes: [
      ...
      { id: "plan", label: "Plan mode", description: "Plan the turn before making changes." },
      {
        ...
        description: "Run without per-command permission prompts.",
      },
...
export const MOCK_AGENT_REPLY =
  "Done. The batch is drained under one lock, so a concurrent scan cannot interleave rows, and the return value lets the caller log the count instead of recomputing it.";
